...
COLORS = {
    ...
    'cyan_concrete': '#25899a', 'smooth_quartz': '#e3e7e5',
    'gray_concrete': '#454c59', 'light_blue_stained_glass': '#87d8e9',
    'purpur_block': '#b998ce', 'purpur_pillar': '#d2b5e5',
}


def render():
    blocks = {p: v for p, v in tower.build_blocks().items() if v[0] != 'minecraft:air'}
    image = Image.new('RGB', (1000, 1200), '#e3e8ec')
    draw = ImageDraw.Draw(image)

    def project(x, y, z):
        return (90 + x * 16 + z * 10, 990 + x * 4 - z * 6 - y * 15)
    ...
    draw.text((30, 25), 'SILPH COMPANY | 32 x 57 x 32 blocks', fill='#354047')
    draw.text((30, 45), 'Authored structure geometry / schematic colors', fill='#59646b')
    output = tower.ROOT / 'docs/assets/silph_company_preview.png'
