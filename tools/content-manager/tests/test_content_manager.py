...
class ContentManagerTests(unittest.TestCase):
    ...
    def test_structure_builder_web_controls_are_present(self) -> None:
        ...
        self.assertIn("function settlementFacilityOptionsCatalog()", script)
        self.assertIn('basic_building_1: "기본 건물 1"', script)
        self.assertIn('train_station: "기차역"', script)
        self.assertIn("function facilityDisplayLabel(id, configuredLabel", script)
        self.assertIn("const requirementsById = new Map(requirements", script)
        self.assertIn('metadata?.category !== "placeholder"', script)
        self.assertIn('note: `NBT 건물 · ${structure}`', script)
        self.assertIn("현재 마을에 포함된 건물", script)
        self.assertIn("settlementFacilityOptionsCatalog().filter((facility)", script)
        self.assertIn('id="open-facility-picker"', markup)
        self.assertIn('id="facility-picker-dialog"', markup)
        self.assertIn("function renderFacilityPicker()", script)
        self.assertIn("function updateFacilitySelection(facilityId, shouldAdd)", script)
        self.assertIn("async function openFacilityPicker()", script)
        self.assertIn("await loadBuildingSettingsData(false)", script)
        self.assertIn('facility-picker-item${selected ? " is-selected" : ""}', script)
        self.assertIn('data-remove-facility="${escapeHtml(facility.id)}"', script)
