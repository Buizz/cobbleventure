...
/** Opens the fossil restoration screen after the authored V5 dialogue completes. */
public final class FossilEventCommandAdapter implements EventCommandAdapter {
    private final java.util.function.Consumer<EventSessionKey> gateway;
    private final EventCommandAdapter fallback;

    public FossilEventCommandAdapter(
    ...
    public StartResult start(CommandContext context) {
        ...
        if (!"command".equals(instruction.operation())
            || !"open_fossil".equals(instruction.command())) {
        ...
        if (instruction.awaitsResult()
            ...
            throw new EventRuntimeException(
                "open_fossil는 await, 결과 변수 또는 안정 ID를 사용하지 않습니다."
            );
        }
        ...
        if (arguments == null || !arguments.isEmpty()
            ...
            throw new EventRuntimeException("open_fossil는 인자와 속성을 받지 않습니다.");
        }
