...
인게임에서 현재 플레이어 파티를 왼쪽 엔트리, 프로젝트에서 생성한 칸나 엔트리를
오른쪽 엔트리로 두고 AI 전투를 직접 확인하려면 OP 권한으로 다음 명령을 실행한다.

```text
/ai_battle_test
```

`/aibattletest`도 같은 명령의 별칭이다. 칸나 트레이너에는 콘텐츠 빌드에서 생성한
`rctmod:kanto_elite_1`과 Cobbleventure AI가 연결되어 있어야 한다. 테스트용 임시
트레이너 엔티티와 런타임 등록은 전투 종료 시 자동으로 제거된다.
