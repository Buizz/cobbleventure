...
Z·메가·다이맥스 연구원과 능력치 연구원 두 자리를 `fixed_npcs`에 등록했다.
능력치 연구원 두 명은 동일한 개체값·노력치 편집 서비스를 제공한다.
로비 안내원과 기술머신 연구원 앵커는 후속 배정용으로 남겨 둔다.
구현 범위·비용·검증 사항은 [연구 서비스 계획](LABORATORY_RESEARCH_PLAN.md)을 따른다.
